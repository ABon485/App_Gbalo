...
const SearchResult = () => {
  ...
  const [selectedProvinceId, setSelectedProvinceId] = useState<string>(
    typeof initialProvinceId === "string" ? initialProvinceId : "all"
  );
  ...
  const fetchTours = async () => {
    try {
      ...
      const fetchedTours: TourItem[] = responseTours.data.datas.map(
        (item: any) => {
          let provinceIds = [];
          ...
          return {
            ...
            provinceName: item.provinceName || "", // Add provinceName to satisfy the type
    ...
    try {
      ...
      if (!keyword.trim()) {
        ...
        suggestionsData = [...provinces, ...destinations]
          ...
          .slice(0, 12);
        console.log(
          `Fetched ${suggestionsData.length} initial suggestions (provinces + destinations)`
        );
      } else {
        ...
        suggestionsData = allProvinces
          ...
          .slice(0, 12);
        console.log(
          `Found ${suggestionsData.length} province suggestions for "${keyword}"`
        );
      }
  ...
  const searchTours = async (
    ...
    try {
      ...
      console.log(
        `Searching for tours: provinceId=${provinceId}, keyword=${keyword}`
      );
      ...
      const provinceIdNum =
        provinceId !== "all" ? parseInt(provinceId, 10) : null;
      ...
      if (filters.priceRange) {
        filteredTours = filteredTours.filter(
          (tour) =>
            tour.fromPrice >= filters.priceRange.min &&
            tour.fromPrice <= filters.priceRange.max
        );
      }
      ...
      if (filteredTours.length === 0) {
        console.log(
          "No matches with province filter, trying keyword-only search"
        );
      ...
      setError(
        error instanceof Error ? error.message : "Không tải được danh sách tour"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSuggestionPress = (suggestion: ProvinceType) => {
    console.log(
      `Selected suggestion: ${suggestion.name} (ID: ${suggestion.id}, Type: ${suggestion.type})`
    );
    ...
    if (suggestion.type === "dest") {
      ...
      searchTours(suggestion.id.toString(), suggestion.name, {
        isDestination: false,
      });
    }
  };
      ...
      <View>
        <Text style={styles.price}>
          ...
          <Text style={styles.priceHighlight}>
            {(item.fromPrice || 0).toLocaleString()}đ/
          </Text>{" "}
          người
        </Text>
      </View>
